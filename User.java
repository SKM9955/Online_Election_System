...
public class User extends HttpServlet
{
	public void doPost(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException
	{
		...
		try{
			...
			ResultSet rs=stmt.executeQuery("select * from verify where EMAIL='"+email+"' and PASSWORD='"+pass+"'");
			if(rs.next())
			{
				...
				out.println("<a href='election.html' style='text-align:center; width:180px; height:60px; line-height:60px; border-radius:10px; background-color:#009100; box-shadow:0 0 10px black; color:white; font-size:20px;'>Okay</a>");
				out.println("</div>");
			...
			else
			{
				...
				out.println("<p style='margin-top:50%; color:white; font-size:20px;'>Login Unsuccessfull</p>");
				
				out.println("<a href='user.html' style='text-align:center; width:180px; height:60px; line-height:60px; border-radius:10px; background-color:red; box-shadow:0 0 10px black; color:white; font-size:20px;'>Okay</a>");
				out.println("</div>");
